...
public class FAGEC extends Plugin implements NewAlgorithm{
	...
	public Vector[] getClusters(String[] para) {
		...
		if(network==null) return null;
		    ...
		    for(int i=0;i<complexes.length;i++){
		        v[i] = new Vector();  
			   for(Object index :complexes[i].getALNodes()){				   
		    	   v[i].add(GraphInfo.nodemap.get(network.getAlNodes().get(((Integer)index).intValue()).getIdentifier()));
			   }
		    }
		   return v;
	}
