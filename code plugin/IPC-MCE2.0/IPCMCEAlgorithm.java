...
public class IPCMCEAlgorithm {
	...
	public IPCMCEAlgorithm(double door){
		this.door = door;
		readFile();
	...
	public Vector[] getResult(){
		...
		int i = 0;
		for(Vector<Node> v : result){
			cc[i] = new Vector<Node>();
			for(Node n : v){
				cc[i].add(GraphInfo.nodemap.get(n.getNodeID()));
			}
			i++;
		}
		return cc;
	...
	public void readFile(){
		...
		for(int i=0;i<edges.size();i++){
			...
			tempNode.getMyNeighbour().add(tempNode2);
			tempNode2.getMyNeighbour().add(tempNode);
	...
	public void getHubs(){
		...
		for(int i=0;i<mynodes.size();){
			...
			MyNode node = mn.getMyNeighbour().get(0);			//获取其唯一邻居节点
			...
			//	for(int j=0;j<node.getNeighbours().size();j++){		//扫描其唯一邻接点 
				for(int j=0;j<node.getMyNeighbour().size();j++){		//扫描其唯一邻接点 
					if(node.getMyNeighbour().get(j).getDegree()==1){
					//	 if(mynodes.remove(node.getNeighbours().get(j)))
						if(mynodes.remove(node.getMyNeighbour().get(j)))
						 {
							 temp.add(node.getMyNeighbour().get(j));          //将度为1的点移除
							 myedges.remove(node.getName()+node.getMyNeighbour().get(j).getName()); //将相应的边移除
						 }
					}
				}
			}
...
//		PP.removeAll(u.getNeighbours());
		...
		while(it.hasNext()){
			...
			RR.addAll(R);
			RR.add(node);
			Bronkerboschl(RR, getUnion(PP2, node.getMyNeighbour()), getUnion(XX, node.getMyNeighbour()));
			PP2.remove(node);
			XX.add(node);
		}
	}
	...
	public void expandClique(){
		...
		while(it.hasNext()){           		//依次对每个极大团进行扩展
			...
			neis.clear();
			while(it2.hasNext()){
				neis.addAll(it2.next().getMyNeighbour());   //获取该极大团中所有的邻居节点
			}
